...
interface FFmpegCoreModule {
    ...
    ret: number;
    reset: () => void;
    setLogger: (fn: (data: LogEvent) => void) => void;
    setProgress: (fn: (data: ProgressEvent) => void) => void;
}

interface FFmpegCoreFactory {
    (options?: { mainScriptUrlOrBlob?: string }): Promise<FFmpegCoreModule>;
}

declare const self: DedicatedWorkerGlobalScope & {
    createFFmpegCore?: FFmpegCoreFactory;
};
 ...
 * Load FFmpeg core
 */
async function load(data: LoadMessageData | undefined): Promise<boolean> {
    if (ffmpeg) {
    ...
    // Set up logging
    ffmpeg.setLogger((data: LogEvent) => {
    ...
    // Set up progress
    ffmpeg.setProgress((data: ProgressEvent) => {
        if (currentMessageId !== null) {
            respond({
                id: currentMessageId,
 ...
 * Convert PCM to MP3
 */
function convert(data: ConvertMessageData): Uint8Array {
    if (!ffmpeg) {
    ...
    // Cleanup
    try { ffmpeg.FS.unlink(inputFile); } catch { /* ignore */ }
    try { ffmpeg.FS.unlink(outputFile); } catch { /* ignore */ }
    ...
    try {
        switch (type) {
            case MessageType.LOAD: {
                const result = await load(data as LoadMessageData);
                respond({ id, type: MessageType.RESULT, data: result ? 'loaded' : 'already_loaded' });
                break;
            }

            case MessageType.CONVERT: {
                const mp3Data = convert(data as ConvertMessageData);
